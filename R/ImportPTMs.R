...
#' @param pattern_isoforms NULL or a character vector of length 1. If not NULL, this function will import the different isoforms as separate rows identified with the specified pattern (as example: "___1", "___2", "___3").
#'
  ...
  if (!is.null(fasta_database)) {
    if (!is.na(fasta_database)) {
      if (fasta_database == "human") {
        fasta_database_loaded <- read_tsv(system.file("extdata", "Database_Human_ref.txt.gz", package = "GetCoolProteopipe"), show_col_types = FALSE)
      } else if (fasta_database == "mouse") {
        fasta_database_loaded <- read_tsv(system.file("extdata", "Database_Mouse_ref.txt.gz", package = "GetCoolProteopipe"), show_col_types = FALSE)
